...
exclusion wording). Formal lock = merge of the design PR.** Increment progress: **F-A landed** (`solve_daily_lp` cap + min-FEC tie-break + `success` key + pins §6-1/2/3/9); **F-B landed** (`cycle_frontier.compute_cycle_cap_frontier` pure calc + pins §6-4..8/10). No code lands until locked, per the house playbook
