...
export-provenance candidate. Step 4 housekeeping and the frontier/floor
follow-ups are merged through [#95](https://github.com/crashchen/euro-bess-radar/pull/95)
as `cc6b1e2`. The #94 [archived snapshot](2026-09-22-frontier.md) and
#95 [archived snapshot](2026-09-22-floor-followup.md) remain revision-bound.
...
|---|---|---|
| Implemented; awaiting review | Multi-day replay and forecast-policy solver provenance | The candidate snapshots each panel's actual MILP dispatch basis in its saved Excel assumptions. Multi-day exports describe the CapEx input to ex-post degradation cost rather than inheriting `Payback period only`. Global Data Trust assumptions, solver inputs and cash stay unchanged. See the [current snapshot](current.md) |
...
| Implemented; awaiting review | Shared assumption labels | The candidate centralizes `Dispatch model` and `CapEx` labels with the global assumptions builder and cockpit export adapters. A focused regression changes both labels and verifies that no old global row survives beside a corrected panel row. See the [current snapshot](current.md) |
