...
repo=Path.cwd()
tracked=subprocess.check_output(['git','diff','--name-only','6d986e3'],text=True).splitlines()
paths=[Path(x) for x in tracked if x.endswith('.md')]
paths += sorted(Path('docs/validation').glob('*.md'))
for candidate in ['docs/audits/2026-09-22-frontier-handoff.md','docs/audits/2026-09-22-frontier-evidence/README.md']:
