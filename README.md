...
- **Ancillary services integration** — manual CSV upload + auto-fetch for DE, FI, GB, RO, SE_3, and Italian zones with per-product reserve lines, manual-over-auto product overrides, and preserved directional/system price fields; plus unified zone-tagged reserve-capacity, activation-energy, and reBAP/imbalance CSV imports as separate streams, each with SQLite persistence and provenance; German reBAP/NRV imbalance data can also be fetched live from Netztransparenz.de into the same cache, and German activated aFRR/mFRR energy (Netztransparenz volumes joined with ENTSO-E 17.1.f activation prices) can be fetched live into the activation cache with dropped-unpriced-interval accounting persisted to provenance
...
| Netztransparenz.de | Germany | Public CSV download handler for DE_LU reBAP + NRV-Saldo and quality-assured activated aFRR/mFRR volumes (joined with ENTSO-E 17.1.f prices for the live activation-energy fetch) |
...
The September corrections (Steps 1–4, #86–#93), [frontier result identity](docs/audits/2026-09-22-frontier-handoff.md) (#94), [frontier/floor export and retry state](docs/audits/2026-09-22-floor-followup-handoff.md) (#95), and [multi-day/forecast-policy Excel provenance](docs/audits/2026-09-22-cockpit-export-handoff.md) (#96, `175c6a7`) are merged. The [current verification snapshot](docs/validation/current.md) is tied to this branch's Cockpit strategy-name XLSX readability candidate. The [#96 snapshot](docs/validation/2026-09-22-cockpit-export.md) and earlier records remain archived at their original revisions.
...
- **Saved results:** multi-day and forecast-policy panels retain complete result and export-assumption snapshots across reruns and downloads. Content changes to inputs consumed by that panel, including same-row-count price corrections and forecast training-history edits, hide stale outputs until Run; restoring the original inputs restores the saved result. The merged frontier fix hashes the full DA frame and every selected date, includes its model version and live consumed constants, and saves export assumptions when Run succeeds. Corrections outside the selected dates conservatively invalidate it too. Reverting inputs restores the saved frontier without solving; a dependent floor result cleared by stale frontier inputs still needs its own Run. Merged #95 makes frontier/floor Excel rows name their actual DA-only MILP, capture and wear basis, saves floor provenance at Run, and clears a previous success on a failed explicit retry. Merged #96 corrects multi-day/forecast-policy Excel solver and wear descriptions. This review branch wraps long strategy names in the Cockpit comparison export. The single-day panel still solves on rerun.
