...
The September corrections (Steps 1–4, #86–#93) and the [frontier result-identity follow-up](docs/audits/2026-09-22-frontier-handoff.md) (#94, `bd4bb88`) are merged. The [current verification snapshot](docs/validation/current.md) is dated and tied to its recorded code head. This branch contains a separate frontier/floor export-provenance and failed-retry follow-up awaiting independent review; the #94 [verification snapshot](docs/validation/2026-09-22-frontier.md) and earlier 2026-09-20 record remain archived.
...
- **Saved results:** multi-day and forecast-policy panels retain complete result and export-assumption snapshots across reruns and downloads. Content changes to inputs consumed by that panel, including same-row-count price corrections and forecast training-history edits, hide stale outputs until Run; restoring the original inputs restores the saved result. The merged frontier fix hashes the full DA frame and every selected date, includes its model version and live consumed constants, and saves export assumptions when Run succeeds. Corrections outside the selected dates conservatively invalidate it too. Reverting inputs restores the saved frontier without solving; a dependent floor result cleared by stale frontier inputs still needs its own Run. On this review branch, frontier/floor Excel assumption rows name the actual DA-only MILP, capture and wear basis; floor exports save that provenance at Run. An explicit failed frontier or floor retry clears the previous success. The single-day panel still solves on rerun.
