...
The September corrections are merged: Step 1 / 1b / 2 in #86–#88, Step 3A / 3B / 3C / 3D in #89–#92, and Step 4 documentation housekeeping in #93 (`6d986e3`). The [current verification snapshot](docs/validation/current.md) remains dated and tied to its recorded code head. The [frontier result-identity follow-up](docs/audits/2026-09-22-frontier-handoff.md) is implemented on the review branch and awaiting review; the current snapshot records that candidate, while the prior 2026-09-20 verification is archived separately.
...
- **Saved results:** multi-day and forecast-policy panels retain complete result and export-assumption snapshots across reruns and downloads. Content changes to inputs consumed by that panel, including same-row-count price corrections and forecast training-history edits, hide stale outputs until Run; restoring the original inputs restores the saved result. The frontier follow-up now hashes the full DA frame and every selected date, includes its model version and live consumed constants, and saves export assumptions when Run succeeds. Corrections outside the selected dates conservatively invalidate it too. Reverting inputs restores the saved frontier without solving; a dependent floor result cleared by stale frontier inputs still needs its own Run. This change awaits review. The single-day panel still solves on rerun.
