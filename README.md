...
- **Ancillary services integration** — manual CSV upload + auto-fetch for DE, FI, GB, RO, SE_3, and Italian zones with per-product reserve lines, manual-over-auto product overrides, and preserved directional/system price fields; plus a unified zone-tagged reserve-capacity CSV import with SQLite persistence and per-(zone, product, direction) provenance, and a unified activation-energy import template (the energy leg of reserves; parser lands in a follow-up increment)
...
├── tests/                    # 604 passing tests, heavily mocked; 2 PDF tests may skip
