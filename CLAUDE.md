...
- **Multi-day SoC carry-over** (`simulate_replay_batch`, DA-only): `carry_soc=True` (default) routes to `_simulate_continuous_da_replay`, which groups the requested local dates into maximal contiguous NaN-free runs and solves each run as ONE `solve_daily_lp` call — terminal-neutral equality applies only at the END of the run, so day N can end with stored SoC that day N+1 monetises. Per-day rows are reconstructed by slicing the run's `p_charge` / `p_discharge` / `soc` arrays at day boundaries; `soc_start_pct` / `soc_end_pct` are real per-day boundaries within the run, not 50% resets. `batch.attrs["carry_mode"] = "continuous_horizon"` labels this regime in the UI; runs are broken by a missing day, an empty slice, any NaN price, or a **sparse day** (`_is_regular_utc_day` rejects a day whose UTC index has a non-uniform interval delta — e.g. a missing 02:00 the upstream cleaner did not reindex to NaN — which would otherwise let the single-`dt` continuous MILP silently compress the time axis). The check is DST-safe: spring-forward (23 local hours) and fall-back (25 local hours) days stay uniform in UTC and are kept. Each run boundary acts as a soft reset and the next run starts at the end-SoC of the previous run. `carry_soc=False` (or single-day `len(dates) < 2`) keeps the legacy per-day terminal-neutral loop and reports `carry_mode = "per_day_reset"`. **Important caveat**: `mode="DA + IDA1 Replay"` always uses the per-day loop today — a continuous-horizon two-stage DA+ID solver is on the roadmap but not implemented. `carry_soc=True` is silently downgraded for IDA mode and `batch.attrs["da_id_carry_soc_supported"] = False` so the UI surfaces a warning. The cockpit also renders a rolling 7-day revenue+FCE curve (≥3 days) and a weekday × ISO-week revenue heatmap (≥7 days) on top of the daily KPI bars to expose weekday/seasonal structure single-day KPIs hide. **Regression guard**: `tests/test_simulation.py::test_replay_batch_carry_soc_produces_higher_revenue_than_reset` constructs a price path where overnight arbitrage is strictly profitable; a no-op carry-over implementation (e.g. only mutating `soc_init_frac` between two independent terminal-neutral daily solves) would tie revenue with the reset mode and fail this assertion.
