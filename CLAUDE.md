...
- **Simulation Cockpit** (`src/simulation.py`, `src/pages/simulation_cockpit.py`): single-local-day, interval-level replay of the existing MILP dispatch model. It reports SoC, physical dispatch, cumulative revenue, throughput/FCE, trade blocks, C-rate, DoD, and screening-level degradation. Treat it as a historical/forward replay cockpit, not live telemetry or real trading execution. Reserve headroom overlay is physical uncommitted headroom only; do not present it as FCR/aFRR activation power without activation data.
...
- `python -m pytest tests/ -v` — run all tests (394 passing tests, fully mocked, no network; 2 PDF chart-render tests may skip when local Kaleido is unavailable).
