...
- **Multi-day SoC carry-over** (`simulate_replay_batch`): `carry_soc=True` (default) chains each day's terminal SoC into the next day's `soc_init_frac`, so multi-day aggregates reflect continuous operation instead of nightly resets to 50%. Excluded days (missing prices, empty intraday merge, etc.) do NOT advance the carried SoC — the next valid day picks up from the last completed day. `carry_soc=False` reproduces the legacy "each day resets to 50% SoC" behaviour and is intentionally biased downward for evening-peak revenue. Batch output exposes `soc_start_pct`/`soc_end_pct` per day and `batch.attrs["carry_soc"]` so the UI can label the regime; the cockpit page also renders a rolling 7-day revenue+FCE curve (≥3 days) and a weekday × ISO-week revenue heatmap (≥7 days) on top of the daily KPI bars to expose weekday/seasonal structure that single-day KPIs hide.
