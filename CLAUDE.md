...
- **Sequential DA+ID policy (cockpit Phase 7)** (`dispatch.solve_sequential_da_id_dispatch`, `ida_forecast.build_ida_forecast`, `simulation.simulate_sequential_da_id_batch`): a sequential-heuristic realisation of the Phase-2 idea, surfaced in the Simulation Cockpit's "Forecast-driven IDA policy" expander. The solver runs Stage 1 (commit DA position via the DA-only MILP), Stage 2 (re-dispatch optimally against an IDA *forecast*, not the realised print), then settles the executed schedule at the *realised* IDA price with the same implicit-MtM accounting as the ex-post solver. It returns a three-way decomposition — `da_only_revenue_eur` (baseline), `realised_total_eur` (forecast-driven), `ceiling_total_eur` (= `solve_daily_da_id_dispatch` perfect-foresight upper bound) — with `forecast_error_cost_eur = ceiling - realised >= 0` and `captured_uplift_eur = realised - da_only` (which **can be negative** when the forecast misleads the rebid into churn losses; do NOT clamp it). Identity `captured + forecast_error == ceiling - da_only` always holds. The forecast is a leave-one-day-out hour-of-day (default, robust on short windows) or hour-of-week (weekday-aware, needs several weeks) climatology mean of the loaded IDA history; `build_ida_forecast` reports `coverage` / `n_buckets_filled` (history-backed buckets) / `n_buckets_requested` / `fallback_points` so the UI labels forecast support honestly. **Framing caveat for any UI/doc copy**: arbitrage timing is driven mainly by price *order/shape*, but a forecast *level* error still changes whether a cycle clears (round-trip efficiency loss + VOM make a level shift flip the cycling decision), so do NOT claim "only shape matters". `simulate_sequential_da_id_batch` aggregates this per-day (standalone terminal-neutral, isolating forecast quality from multi-day SoC carry) and reports a guarded `capture_rate` (None when no rebid opportunity exists in the window). This is a single-stage screening estimate with NO risk-gating/deadband — the unconditional "always follow the forecast" policy can lose small amounts to churn on no-opportunity days, which is intentional and honest. A scenario-based stochastic MILP remains the heavier future upgrade.
